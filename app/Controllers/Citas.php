    ...
    public function comprobarDisponibilidad($idDoctor,$horaInicio,$horaFin,$fecha){
        ...
        if(sizeof($citas)>0){
            foreach($citas as $cita){
                if(date("H:i", strtotime($horaInicio)) >= date("H:i",strtotime($cita['HoraInicio']." ".$cita['Fecha'])) 
                && date("H:i",strtotime($horaInicio)) <=  date("H:i",strtotime($cita['HoraFin']." ".$cita['Fecha']))){
                    return false;
                }
                else if(date("H:i", strtotime($horaInicio)) <= date("H:i",strtotime($cita['HoraInicio']." ".$cita['Fecha'])) 
